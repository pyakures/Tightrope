"""
    ...
    Layout in my mind:
        1. retrieve event data for next week (needs to be sent from the database, Icould cut it off here but it would waste computing power)
        ...
        if event.id != 1:
            ...
            if (event["START_TIME_ID"] - previous_event[END_TIME_ID]) > TIME_THRESHOLD:
                ...
                for preferred_event in event_preferences:
                    if preferred_event["length"] < length_of_event:
